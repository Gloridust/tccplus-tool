...
char* get_app_path() {
    ...
    fgets(app_path, MAX_URL_LENGTH, stdin);
...
char* get_bundle_identifier(const char *app_path) {
    ...
    file = fopen(info_plist_path, "r");
    if (file) {
        char line[MAX_URL_LENGTH];
        while (fgets(line, sizeof(line), file)) {
            if (strstr(line, "<key>CFBundleIdentifier</key>") != NULL) {
                fgets(line, sizeof(line), file);
                char *bundle_identifier = strchr(line, '>') + 1;
                strtok(bundle_identifier, "<"); // Remove trailing "<" character
